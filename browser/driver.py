        ...
        "actions": {
            ...
            "items": {
                ...
                "properties": {
                    "type": {
                        "type": "string",
                        "enum": ["click", "type", "key", "scroll", "drag", "wait", "go_back", "close_tab", "click_new_tab", "switch_tab", "done"],
                    },
                    "mark":   {"type": "integer"},
                    "value":  {"type": "string"},
                    "clear":  {"type": "boolean"},
...
SYSTEM_PROMPT_VISION = (
    ...
    "  close_tab()                — close the current tab/page\n"
    "  click_new_tab(mark)        — click link to open it in a new tab\n"
    "  switch_tab(index)          — switch to the tab at index (from OPEN TABS list)\n"
...
async def _dispatch(action: dict, page: Page, snap: PageSnapshot) -> str:
    ...
    if t == "click_new_tab":
        ...
        if not el:
            return f"error: no element with mark {action.get('mark')!r}"
        await page.mouse.click(el.cx, el.cy, modifiers=["Control"])
        return "ok"
    if t == "done":
        return "ok"
...
class BaseDriver:
    ...
    def _history_text(self) -> str:
        ...
        for s in recent:
            acts = ", ".join(
                f"{a['type']}({a.get('mark') or a.get('value', '') or a.get('index', '')})"
                for a in s.actions[:3]
            )
            lines.append(f"turn {s.turn}: {acts} → {s.outcome}")
        return "\n".join(lines)

    async def _get_open_tabs_text(self) -> str:
        try:
            open_pages = [p for p in self.context.pages if not p.is_closed()]
            lines = []
            for i, p in enumerate(open_pages):
                active_str = " (ACTIVE)" if p == self.page else ""
                try:
                    title = await p.title()
                except Exception:
                    title = "Untitled"
                lines.append(f"  [{i}] {title} — {p.url}{active_str}")
            return "OPEN TABS:\n" + "\n".join(lines)
        except Exception:
            return ""
    ...
    async def step(self, turn: int) -> tuple[bool, bool, str]:
        # Track if multiple tabs/pages exist in this context and switch to the latest open one
        try:
            open_pages = [p for p in self.context.pages if not p.is_closed()]
            if open_pages:
                if self.page not in open_pages or (len(open_pages) > 1 and self.page != open_pages[-1]):
        ...
        for a in actions:
            if a.get("type") == "done":
                ...
                break
            if a.get("type") == "switch_tab":
                try:
                    idx = int(a.get("index", 0))
                    open_pages = [p for p in self.context.pages if not p.is_closed()]
                    if 0 <= idx < len(open_pages):
                        self.page = open_pages[idx]
                        outcome = f"ok: switched to tab {idx}"
                    else:
                        outcome = f"error: tab index {idx} out of range (open tabs: {len(open_pages)})"
            ...
            try:
                pages_before = len([p for p in self.context.pages if not p.is_closed()])
            except Exception:
                pages_before = 1

            try:
                outcome = await _dispatch(a, self.page, snap)
                # Check for new tabs opened/closed during action execution
                try:
                    open_pages = [p for p in self.context.pages if not p.is_closed()]
                    if open_pages:
                        if self.page.is_closed() or len(open_pages) != pages_before:
                            self.page = open_pages[-1]
                except Exception:
                    pass
            except Exception as e:
...
class SetOfMarksDriver(BaseDriver):
    ...
    async def _decide(self, snap: PageSnapshot, turn: int):
        ...
        tabs_text = await self._get_open_tabs_text()
        prompt = (
